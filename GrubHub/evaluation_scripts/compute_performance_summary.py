...
def compute_performance_summary(instance_dir,input_dir,output_dir):
    ...
    for i,a in assignment_sol.iterrows():
        offtime=couriers.at[a.courier,'off_time'] + 2 # just to be safe on rounding errors
        if offtime<a.pickup_time:
            #violations.append((offtime,a.pickup_time))
    ...
    if feasible:
        ...
        with open(output_dir + "/mdrp_metrics.csv", 'w') as myf:
            myf.write(str((len(orders) - total_delivered)/len(orders) * 100))
            myf.write(',')
            myf.write(str(order_performance['click-to-door'].mean()))
            myf.write(',')
            myf.write(str(order_performance['click-to-door'].quantile(0.9)))
            myf.write(',')
            myf.write(str(order_performance['click-to-door overage'].mean()))
            myf.write(',')
            myf.write(str(order_performance['ready-to-pickup'].mean()))
            myf.write(',')
            myf.write(str(order_performance['ready-to-pickup'].quantile(0.9)))
            myf.write(',')
            myf.write(str(courier_performance['utilization'].mean()))
            myf.write(',')
            myf.write(str(courier_performance['utilization'].quantile(0.1)))
            myf.write(',')
            myf.write(str(total_cost/total_delivered))
            myf.write(',')
            myf.write(str(bundle_size['orders_per_bundle'].mean()))

        
        if verbosity==1:
            print ("------------------------- MDRP METRICS -------------------------")
            
            print("%% undelivered " + str((len(orders) - total_delivered)/len(orders) * 100))
            print("CtoD mean = " + str(order_performance['click-to-door'].mean()))
            print("CtoD 90 %% = " + str(order_performance['click-to-door'].quantile(0.9)))
            print("CtoD overage = " + str(order_performance['click-to-door overage'].mean()))
            print("RtoP mean = " + str(order_performance['ready-to-pickup'].mean()))
            print("RtoP 90 %% = " + str(order_performance['ready-to-pickup'].quantile(0.9)))
            print("utilization mean = " + str(courier_performance['utilization'].mean()))
            print("utilization 10 %% = " + str(courier_performance['utilization'].quantile(0.1)))
            print("cost per order = " + str(total_cost/total_delivered))
            print("orders per bundle = " + str(bundle_size['orders_per_bundle'].mean()))
            print ("----------------------------------------------------------------")
